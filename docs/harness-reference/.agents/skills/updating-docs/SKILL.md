...
- TSDoc in `src/**` — a published symbol's contract. A generic type shared across zones
  is where this matters most, `Result` in `src/core/result.ts` being the one every zone
  is written against: the comment is what a caller reads instead of reading every module
  that happens to use it. `writing-typescript` owns what the comment says; this skill
  owns only whether one is owed.
